...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, reduce_factor=2):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, (4//reduce_factor) * dim, bias=False)
...
class TransMorphEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        前向传播函数。
        Args:
            x: 输入图像，形状为 (B, C, H, W)，其中 B 是 batch size，C 是通道数，H, W 是图像的空间维度。
        Returns:
...
if __name__ == '__main__':
    ...
    # 输入图像
    x = torch.randn(1, 2,1024, 1024)  # (B, C, H, W)
